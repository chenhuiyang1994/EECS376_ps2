...
#include <sensor_msgs/LaserScan.h>// /scan is also of this Msg type 
...
float ping_dist_in_front_=1.0; // global var to hold length of a SINGLE LIDAR ping--in front
...
laser_alarm_=false;
   ...
   ROS_INFO("detected dist= %f",ping_dist_in_front_);
   if (ping_dist_in_front_<MIN_SAFE_DISTANCE) {
       ROS_WARN("DANGER, chenhui yang!!");
       laser_alarm_=true;
   }
}
